...
class ElectronicConfiguration(object):
    ...
    def nvalence(self, block, method=None):
        ...
        if block in ['s', 'p']:
            return sum([v for k, v in self.conf.items()
                        if k[0] == self.max_n()])
        elif block == 'd':
            if method == 'simple':
                return 2
    ...
    def slater_screening(self, n, o, alle=False):
        ...
        if o in ['s', 'p']:
            ...
            vale = float(sum([v for k, v in self.conf.items()
                              if k[0] == n and k[1] in ['s', 'p']]) - ne)
            n1 = sum([v * 0.85 for k, v in self.conf.items() if k[0] == n - 1])
            n2 = sum([float(v) for k, v in self.conf.items()
            ...
            n1 = sum([float(v) for k, v in self.conf.items()
                      if k[0] == n and k[1] != o])
            n2 = sum([float(v) for k, v in self.conf.items()
                      if k[0] in range(1, n)])
...
def print_spin_occupations(sodict, average=True):
    ...
    for (n, orb), occ in sodict.items():
        ...
        if average:
            ...
            a = ', '.join(['{0:3.1f}'.format(x)
                           for x in [1] * occ['alpha'] + [0] * (nss - occ['alpha'])])
            b = ', '.join(['{0:3.1f}'.format(x)
